...
def main_process_data(history, tag_row, time_start, time_end):
    ...
    try:
		...
		if vars['event_start'] and not vars['event_end']:
		    vars['event_end'] = time_end
		    vars['event_duration'] = help_get_duration(vars['event_start'], vars['event_end'])
		    vars['stop_reason'] = 'In Progress'
		    event_data = help_prepare_event_data(vars, tag_row)
		    events.append(event_data)
...
def db_write_events(events):
    ...
    try:
        for event in events:
            # Check if the event already exists and its status
            query_check = """
                SELECT id, stop_reason FROM cip_report.dbo.history_events
                WHERE circuit_id = ? AND time_start = ?
            """
            params_check = [event['circuit_id'], event['time_start']]
            existing_event = system.db.runPrepQuery(query_check, params_check, tx=txId)
            
            if existing_event:
                event_id = existing_event[0]['id']
                stop_reason = existing_event[0]['stop_reason']
                
                if stop_reason == 'In Progress':
                    # Update the existing event
                    update_query = """
                        UPDATE cip_report.dbo.history_events
                        SET time_end = ?, duration = ?, stop_reason = ?, pause_count = ?, held_count = ?, advanced_count = ?,
                            pause_duration = ?, held_duration = ?, type_sanitizer = ?, type_acid = ?, type_caustic = ?, type_alkaline = ?, 
                            updated_at = GETDATE(), updated_by = 'Ignition'
                        WHERE id = ?
                    """
                    params_update = [
                        event['time_end'], event['duration'], event['stop_reason'], event['pause_count'], 
                        event['held_count'], event['advanced_count'], event['pause_duration'], 
                        event['held_duration'], event['type_sanitizer'], event['type_acid'], 
                        event['type_caustic'], event['type_alkaline'], event_id
                    ]
                    system.db.runPrepUpdate(update_query, params_update, tx=txId)
                    
                    # Delete existing steps and operation events for the event
                    delete_steps_query = "DELETE FROM cip_report.dbo.history_events_steps WHERE event_id = ?"
                    delete_operator_query = "DELETE FROM cip_report.dbo.history_events_op WHERE event_id = ?"
                    system.db.runPrepUpdate(delete_steps_query, [event_id], tx=txId)
                    system.db.runPrepUpdate(delete_operator_query, [event_id], tx=txId)
                    
                    # Insert new steps and operation events
                    db_write_steps(event['steps'], event_id, txId)
                    db_write_operator(event['operation_events'], event_id, txId)
                    
                    help_send_log_message('CIPReport Write', 'Updated CIP: ' + str(event['time_start']) + ' circuit: ' + str(event['circuit_id']) + ' eventid: ' + str(event_id), show=True)
                else:
                    help_send_log_message('CIPReport Write', 'Event already exists and is not in progress')
            else:
                # Insert a new event
                insert_query = """
